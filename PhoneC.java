...
public class PhoneC extends Frame implements ActionListener { // ActionListerを使ってボタンを反応させる
  ...
  static String strServer; //相手(サーバー)のPCのIPアドレスを入力するためのもの
  ...
  public static void main(String [] args) { // メイン文
    PhoneC ps = new PhoneC(); // PhoneS()を行う
  }

  // ■ コンストラクタ
  PhoneC() {
    super("まぐろ client"); //実行時につく名前
  ...
  public void phone(){ //通話を行う関数
    try{
      ...
      Socket skt = new Socket(strServer, nPort); //ソケットの接続を試みる
    ...
    Rec.end(); //ターゲットデータラインを閉じることができる
        ...
        System.err.println("サーバーが見つかりません。どっか行きました\n" + e); //通話後にネットワークが切断されると、出力される
        System.exit(0);
    }catch(Exception e){
